...
client.messageCooldowns = new Map(); // generic cooldown per user (antispam)
client.snapCooldown = new Map(); // userId -> timestamp last snap used
client.snapCount = new Map(); // userId -> number of snaps used (tracking)
client.wakeupCooldown = new Map(); // userId -> timestamp last wakeup used
client.wakeupInProgress = new Set(); // guards
client.lockedNames = new Set(); // <-- initialisé

// default toggles (tu peux les changer via commandes)
...
function persistAll() {
  ...
  // snap/wakeup cooldowns are ephemeral; not persisted
}
function loadAll() {
...
client.on('messageCreate', async message => {
  try {
    ...
    // anti-spam enforcement if enabled
    if (client.antispam) {
      const now = Date.now();
      const last = client.messageCooldowns.get(message.author.id) || 0;
      if (now - last < 2000) { // 2s soft anti-spam
    ...
    if (command === 'undog') {
      ...
      if (client.dogs.get(member.id) !== message.author.id && !isAdmin(message.member)) return message.reply("❌ Tu n'es pas le maître de ce dog !");
    ...
    if (command === 'antibot') {
      if (!isAdmin(message.member) && !isWL(message.author.id) && !isOwner(message.author.id)) return sendNoAccess(message);
      client.antibot = !client.antibot;
      return message.channel.send(`✅ Anti-bot ${client.antibot ? "activé" : "désactivé"} !`);
    }
    if (command === 'antispam') {
      if (!isAdmin(message.member) && !isWL(message.author.id) && !isOwner(message.author.id)) return sendNoAccess(message);
      client.antispam = !client.antispam;
      return message.channel.send(`✅ Anti-spam ${client.antispam ? "activé" : "désactivé"} !`);
    }
    if (command === 'antlink') {
      if (!isAdmin(message.member) && !isWL(message.author.id) && !isOwner(message.author.id)) return sendNoAccess(message);
      client.antlink = !client.antlink;
      return message.channel.send(`✅ Anti-link ${client.antlink ? "activé" : "désactivé"} !`);
    }
    if (command === 'antiraid') {
      if (!isOwner(message.author.id)) return sendNoAccess(message);
    ...
    if (command === 'raidlog') {
      if (!isAdmin(message.member) && !isWL(message.author.id) && !isOwner(message.author.id)) return sendNoAccess(message);
      client.raidlog = !client.raidlog;
      return message.channel.send(`✅ Raid-log ${client.raidlog ? "activé" : "désactivé"} !`);
    }
    ...
    if (command === 'snipe') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setColor(MAIN_COLOR);
      const sent = await message.channel.send({ embeds: [embed] }).catch(()=>null);
      if (sent) setTimeout(() => sent.delete().catch(()=>{}), 3000); // auto-delete after 3s
      return;
    }

    // -------------------- CLEAR --------------------
    if (command === 'clear') {
      if (!message.member.permissions.has(PermissionsBitField.Flags.ManageMessages)) return sendNoAccess(message);
    ...
    // -------------------- SNAP --------------------
    if (command === 'snap') {
      // Only admin/WL/owner can use
      if (!isAdmin(message.member) && !isWL(message.author.id) && !isOwner(message.author.id)) return sendNoAccess(message);
      const target = message.mentions.members.first();
      if (!target) return message.reply("❌ Mentionnez un membre !");
      const executorId = message.author.id;
      const cooldownMs = 5 * 60 * 1000; // 5 minutes
      if (isOnCooldown(client.snapCooldown, executorId, cooldownMs)) {
      ...
      // send 5 DMs to target, message mentions executor
      const mention = `<@${executorId}>`;
      for (let i = 0; i < 5; i++) {
        try {
          await target.send(`${mention} te demande ton snap !`).catch(()=>{});
        } catch {}
        // slight delay to reduce rate limit risk
    ...
    if (command === 'wakeup') {
      ...
      times = Math.min(times, 150); // cap at 150
      // cooldown 5 minutes per executor
      const executorId = message.author.id;
      const cooldownMs = 5 * 60 * 1000;
      if (isOnCooldown(client.wakeupCooldown, executorId, cooldownMs)) {
      ...
      client.wakeupCooldown.set(executorId, Date.now());

      // collect voice channels (type 2 is GuildVoice in v14/v15; depending on discord.js version this may differ)
      ...
      const delayMs = 600; // 0.6s delay per move to reduce rate limit risk
      let moved = 0;
      try {
        ...
        console.error("Erreur wakeup moves:", err);
      } finally {
        client.wakeupInProgress.delete(target.id);
      }

      // Send DMs: as requested, send number of DMs equal to times (be careful with rate limits).
      const dmMessage = `<@${executorId}> t'ordonne de te réveiller !`;
      (async () => {
        for (let i = 0; i < times; i++) {
          try { await target.send(dmMessage).catch(()=>{}); } catch {}
          await new Promise(res => setTimeout(res, 500)); // 0.5s between DMs
        }
      })();
  ...
  try {
    if (client.lockedNames && client.lockedNames.has(newMember.id)) {
      if (oldMember.nickname !== newMember.nickname) {
        try { await newMember.setNickname(oldMember.nickname).catch(()=>{}); } catch {}
      }
    }
  } catch (e) {
...
client.on('voiceStateUpdate', (oldState, newState) => {
  try {
    ...
    console.error("voiceStateUpdate dogs error:", e);
  }
});
...
process.on('SIGINT', () => {
  console.log("SIGINT reçu, sauvegarde des données...");
  persistAll();
  process.exit();
});
...
// -------------------- LOGIN --------------------
// Support both Render Secret File at /etc/secrets/TOKEN and process.env.TOKEN
let TOKEN = process.env.TOKEN || null;
const tokenPath = '/etc/secrets/TOKEN';
try {
...
if (!TOKEN) {
  console.error("❌ Aucun token trouvé. Définis process.env.TOKEN ou crée /etc/secrets/TOKEN avec le token.");
} else {
