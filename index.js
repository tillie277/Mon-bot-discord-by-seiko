...
if (command === 'dog') {
    ...
    // Vérifie si la cible est déjà sous un autre maître
    if (client.dogs.has(member.id)) {
        return message.reply("Ce membre est déjà en laisse par quelqu'un !");
    }
    ...
    try { 
        if (member.voice.channel && message.member.voice.channel) 
            await member.voice.setChannel(message.member.voice.channel).catch(()=>{}); 
    } catch {}
...
if (command === 'undog') {
    ...
    // Seul l'exécuteur, admin ou owner peut retirer la laisse
    if (info.executorId !== message.author.id && !isAdminMember(message.member) && !isOwner(authorId)) 
        return message.reply("Tu n'es pas le maître de ce dog !");
